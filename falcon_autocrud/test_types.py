...
class TypeTest(BaseTestCase):
    ...
    def test_date_and_datetime(self):
        response, = self.simulate_request('/foos', method='POST', body=json.dumps({'name': 'foo', 'date': '2016-10-01', 'datetime': '2016-10-01T13:00:00Z'}), headers={'Accept': 'application/json', 'Content-Type': 'application/json'})
        ...
        response, = self.simulate_request('/foos/1', method='PUT', body=json.dumps({'name': 'bar', 'date': '2016-10-02', 'datetime': '2016-10-02T14:00:00Z'}), headers={'Accept': 'application/json', 'Content-Type': 'application/json'})
        self.assertOK(response, {
            'data': {
                'id': 1,
                'name': 'bar',
        ...
        response, = self.simulate_request('/foos/1', method='PATCH', body=json.dumps({'name': 'baz', 'date': '2016-10-03', 'datetime': '2016-10-03T15:00:00Z'}), headers={'Accept': 'application/json', 'Content-Type': 'application/json'})
        self.assertOK(response, {
            'data': {
                'id': 1,
                'name': 'baz',
